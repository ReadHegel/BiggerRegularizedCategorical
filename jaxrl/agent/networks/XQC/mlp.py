...
class TanhGaussPredictor(nn.Module):
    ...
    @nn.compact
    def __call__(self, x: jnp.ndarray, training: bool = False) -> distrax.Distribution:
        means = nn.Dense(
        ...
        stds = jnp.exp(log_stds) * self.temperature
...
class MLP(nn.Module):
    ...
    @nn.compact
    def __call__(self, x: jnp.ndarray, training: bool = False) -> Union[jnp.ndarray, distrax.Distribution]:
